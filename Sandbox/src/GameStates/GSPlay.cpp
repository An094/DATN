...
void GSPlay::Init()
{
	m_sprite2d = std::make_shared<Sprite2D>("bg_play.tga");
	m_sprite2d->SetPosition(widthScreen/2, heightScreen/2);
	m_sprite2d->SetSize(widthScreen, heightScreen);

	m_button = std::make_shared<Button>("back_play.tga", "Texture", "Texture", ButtonType::CIRCLE);
	...
	m_spriteAnim = std::make_shared<SpriteAnimation2D>("poo_down.tga", "Texture", "Animation", 6, 0.1f);
